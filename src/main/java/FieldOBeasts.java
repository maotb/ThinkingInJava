...
public class FieldOBeasts extends Applet implements Runnable {
	...
	static float fieldOfView = (float) (Math.PI / 4), // In radians
			// Deceleration % per second:
			decayRate = 1.0f, minimumDistance = 10f; // In pixels
	static int halfFieldOfView = (int) ((fieldOfView * 360) / (2 * Math.PI)), xExtent = 0, yExtent = 0, numBeasts = 50,
	...
	Vector makeBeastVector(int quantity, boolean uniqueColors) {
		...
		for (int i = 0; i < quantity; i++) {
			...
			newBeasts.addElement(new Beast(this, x, y, direction, speed, new Color(r, g, b)));
		}
		return newBeasts;
	}
